...
```{sql, connection=db, output.var="databases"}
SHOW DATABASES;
```
...
```{sql, connection=db, output.var="tables"}
SHOW TABLES;
```

```{sql, connection=db, output.var="yellow"}
DESCRIBE yellow;
```

```{sql, connection=db, output.var="yellowsum"}
DESCRIBE yellow_sum;
```

```{sql, connection=db, output.var="yellowoldsum"}
/*DESCRIBE yellow_old_sum;*/
```


```{sql, connection=db, output.var="first_hundred_yellow"}
/*SELECT trip_distance, passenger_count, fare_amount, tip_amount, total_amount*/
SELECT *
FROM yellow 
LIMIT 100;
```
